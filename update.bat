...
if "%TGZ%"=="" if not "%RELEASE_BASE%"=="" (
  ...
  for /f "usebackq delims=" %%u in (`node -e "try{var https=require('https'),http=require('http');var base=process.argv[1].replace(/\/+$/,'');var get=function(url,cb){var mod=url.indexOf('https:')===0?https:http;var req=mod.get(url,function(res){if(res.statusCode!==200){res.resume();cb(null);return;}var b='';res.on('data',function(d){b+=d});res.on('end',function(){cb(b)})});req.on('error',function(){cb(null)});req.setTimeout(20000,function(){req.destroy();cb(null)});};get(base+'/releases.json',function(body){if(!body){process.stdout.write('');return;}try{var m=JSON.parse(body);var f='';if(m.downloads&&m.downloads.tarball)f=m.downloads.tarball;else if(m.files){for(var i=0;i<m.files.length;i++){if(m.files[i].file&&/\.tgz$/.test(m.files[i].file)){f=m.files[i].file;break;}}}process.stdout.write(f?base+'/'+f:'');}catch(e){process.stdout.write('');}});}catch(e){process.stdout.write('');}" "%RELEASE_BASE%" 2^>nul`) do set "DL_URL=%%u"
  if not "!DL_URL!"=="" (
